((window, red5prosdk, SubscriberBlock) => {
  ...
  const appContext = window.getParamByName('app') || `nextechar`
  ...
  const roomName = window.getParamByName('room') || 'room1'
  const SM_ACCESS_TOKEN = window.getParamByName('smtoken') || '2IzJQoyMXIc9'
  const ws = window.getParamByName('ws') || 'localhost:8001'
  ...
  const userid = window.getParamByName('userid') || 'default-userid'
  const permissions = window.getParamByName('permissions') || '[]'
...
const getWaitingRoomContext = () => {
  return `${appContext}/${roomName}_wr`
}
*/
  ...
  const setUpStreamListSocket = (url) => {
    ...
    */
    ...
    websocket.onmessage = event => {
      ...
      if (json.type === 'conference' && json.room === `${appContext}/${roomName}`) {
  ...
  const postProvisionOrigin = async (provision) => {
    const host = baseConfig.host
    const port = secureConnection ? '443' : 5080
    const protocol = secureConnection ? 'https' : 'http'
    const baseUrl = protocol + '://' + host + ':' + port
    const provisionUrl = baseUrl + '/cluster/api?action=provision.create'
  ...
  const postProvision = async (provision) => {
    try {
      if (requiresStreamManager) {
        ...
        await postProvisionOrigin(provision)
      }
    } catch (e) {
      throw e
    }
